...
public class StatScreen1 extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        Text[] texts = {Text.literal("Status "+(speed.getValue()/speed.getBaseValue())*100+"%"),Text.literal("Information").styled(style -> style.withUnderline(true)),Text.literal("[").append(Text.literal("Name")).append(": "+nbt.getString("first_name")+" "+nbt.getString("last_name")+"]"),
                Text.literal("[Health: "+(double) Math.round(player.getHealth()*10)/10+"/"+player.getMaxHealth()+"]"),Text.literal("[Level: "+nbt.getInt("total_level")+" ("+nbt.getInt("experience")+"/"+nbt.getInt("max_experience")+")"+"]"),
                Text.literal("[Hit Dice :"+nbt.getInt("hit_dice")+"]"),Text.literal("[Proficiency Bonus: "+nbt.getInt("proficiency_modifier")+"]"),Text.literal("[Ki Points: "+nbt.getInt("ki")+"]"),
                Text.literal("Class List").styled(style -> style.withUnderline(true)),Text.literal("Stats").styled(style -> style.withUnderline(true)),Text.literal("Str"+"- "+nbt.getIntArray("stats")[0]),Text.literal("Dex"+"- "+nbt.getIntArray("stats")[1]),
                Text.literal("Con"+"- "+nbt.getIntArray("stats")[2]),Text.literal("Int"+"- "+nbt.getIntArray("stats")[3]),Text.literal("Wis"+"- "+nbt.getIntArray("stats")[4]),Text.literal("Cha"+"- "+nbt.getIntArray("stats")[5])};
