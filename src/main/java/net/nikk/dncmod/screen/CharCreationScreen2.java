...
public class CharCreationScreen2 extends Screen {
    ...
    public void render(DrawContext matrices, int mouseX, int mouseY, float delta) {
        ...
        textRendererMatrixStack.drawText(textRenderer, Text.literal("Pick a ").styled(style -> style.withBold(false).withItalic(false)).append("class").styled(style -> style.withBold(true).withItalic(true)).append(", any ").append("class").styled(style -> style.withBold(true).withItalic(true)).append(" from the following options:"), x+collum*4, y+20+line*3+line, 	15859709, false);
    ...
    private void DrawClass(String id,DrawContext matrices,DrawContext textRendererMatrixStack,int[] pic,int[] textX,int[] textY,Text[] texts,int[] max){
        matrices.drawTexture(new Identifier(DNCMod.MOD_ID, "textures/gui/"+id+".png"), pic[0] + pic[1] * 25 / 2, pic[2] + 20 + pic[3] * 5 + pic[3], 0, 0, 90, 90, 90, 90);
        for (int i:max) textRendererMatrixStack.drawText(textRenderer, texts[i], pic[0] + pic[1] * textX[i] / 2 + 3, pic[2] + 20 + pic[3] * textY[i] + pic[3], 15859709, false);
