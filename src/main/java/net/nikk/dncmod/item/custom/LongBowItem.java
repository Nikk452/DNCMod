...
public class LongBowItem extends BowItem {
    ...
    public static final Predicate<ItemStack> LONGBOW_PROJECTILES = (stack) -> stack.isOf(ModItems.LONG_ARROW);

    @Override
    public Predicate<ItemStack> getProjectiles() {
        return LONGBOW_PROJECTILES;
    }
    @Override
        ...
        if (user instanceof PlayerEntity playerEntity) {
            ...
            if (!itemStack.isEmpty() || bl) {
                if (itemStack.isEmpty()) {
                    itemStack = new ItemStack(ModItems.LONG_ARROW);
                }

                int i = this.getMaxUseTime(stack) - remainingUseTicks;
                float f = getPullProgress(i);
                if (!((double)f < 0.1)) {
                    boolean bl2 = bl && itemStack.isOf(ModItems.LONG_ARROW);
                    if (!world.isClient) {
                        ArrowItem arrowItem = (ArrowItem)(itemStack.getItem() instanceof ArrowItem ? itemStack.getItem() : ModItems.LONG_ARROW);
